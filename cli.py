...
@click.command(help="Generates a PDF report with plots, tables, and correlation data")
@click.option('--report', '-r', type=click.Path(writable=True, dir_okay=False), default='report.pdf', help='Output PDF report file name')
@click.option('--abbr_data', '-ad', type=click.Path(exists=True, dir_okay=False, readable=True), default='abbreviation_data.csv', help='Input CSV file containing abbreviation data')
@click.option('--obs_plot', '-op', type=click.Path(exists=True, dir_okay=False, readable=True), default='observation.png', help='Input PNG file for the observation plot')
@click.option('--cond_plot', '-cp', type=click.Path(exists=True, dir_okay=False, readable=True), default='condition.png', help='Input PNG file for the condition plot')
@click.option('--epi_curve', '-ec', type=click.Path(exists=True, dir_okay=False, readable=True), default='epidemic_curve.png', help='Input PNG file for the epidemic curve plot')
@click.option('--histogram_dir', '-hgdir', type=click.Path(exists=True, file_okay=False, readable=True), default='histograms', help='Directory containing histogram PNG files')
@click.option('--crosstab', '-ct', type=click.Path(exists=True, dir_okay=False, readable=True), default='crosstab.csv', help='Input CSV/XLSX file for the crosstab table')
@click.option('--freq', '-ft', type=click.Path(exists=True, dir_okay=False, readable=True), default='freq_table.csv', help='Input CSV file for frequency table')
@click.option('--attack_rate', '-at', type=click.Path(exists=True, dir_okay=False, readable=True), default='attack_rate.csv', help='Input CSV file for attack rate table')
@click.option('--corr_plot', '-corr', type=click.Path(exists=True, dir_okay=False, readable=True), default='corr.png', help='Input PNG file for the correlation plot')
def generate_report(report, abbr_data, obs_plot, cond_plot, epi_curve, histogram_dir, crosstab, attack_rate, freq, corr_plot):
    pdf = FPDF()
    ...
    pdf.cell(200, 10, txt="ADARV STATS", ln=True, align='C')
    ...
    if abbr_data:
        ...
        pdf.cell(200, 10, txt="Abbreviation Data", ln=True, align='L')
        pdf.ln(10)
        
        df_abbr = pd.read_csv(abbr_data)
        col_width = 170 / len(df_abbr.columns)  
        th = pdf.font_size + 2
        padding = 1.5

        pdf.set_font("Arial", 'B', 10)
        for col in df_abbr.columns:
            pdf.cell(col_width, th, col, border=1, align='C')
        pdf.ln(th + padding)

        pdf.set_font("Arial", '', 10)
        for i, row in df_abbr.iterrows():
            for value in row:
                pdf.cell(col_width, th, str(value), border=1, align='C')
            pdf.ln(th + padding)
        pdf.ln(10)

    if obs_plot:
        pdf.set_font("Arial", 'B', 16)
        pdf.cell(200, 10, txt="Observation Plot", ln=True, align='L')
        pdf.image(obs_plot, x=15, y=None, w=180)
        pdf.ln(15)

    if cond_plot:
        pdf.set_font("Arial", 'B', 16)
        pdf.cell(200, 10, txt="Condition Plot", ln=True, align='L')
        pdf.image(cond_plot, x=15, y=None, w=180)
        pdf.ln(15)

    if epi_curve:
        pdf.set_font("Arial", 'B', 16)
        pdf.cell(200, 10, txt="Epidemic Curve", ln=True, align='L')
        pdf.image(epi_curve, x=15, y=None, w=180)
        pdf.ln(15)
    ...
    if histogram_dir:
        pdf.set_font("Arial", 'B', 16)
        pdf.cell(200, 10, txt="Histograms", ln=True, align='L')
        pdf.ln(10)

        histograms = [f for f in os.listdir(histogram_dir) if f.endswith('.png')]
        x_positions = [15, 105]
        y_position = 30
        for i, histogram in enumerate(histograms):
            x = x_positions[i % 2]
            pdf.image(os.path.join(histogram_dir, histogram), x=x, y=y_position, w=85)
            if i % 2 == 1:
                y_position += 70
            if (i + 1) % 6 == 0:
                pdf.add_page()
    ...
    if corr_plot:
        pdf.add_page()
        pdf.set_font("Arial", 'B', 16)
        pdf.cell(200, 10, txt="Correlation Plot", ln=True, align='L')
        pdf.image(corr_plot, x=10, y=None, w=190)  # Increase width to 190
        pdf.ln(15)

    if crosstab:
        pdf.add_page()
        pdf.set_font("Arial", 'B', 16)
        pdf.cell(200, 10, txt="Crosstab Table", ln=True, align='L')
        pdf.ln(10)

        if crosstab.endswith('.csv'):
            df_crosstab = pd.read_csv(crosstab)
        else:
            df_crosstab = pd.read_excel(crosstab)

        col_width = 170 / len(df_crosstab.columns)  
        th = pdf.font_size + 2
        padding = 1.5

        pdf.set_font("Arial", 'B', 10)
        for col in df_crosstab.columns:
            pdf.cell(col_width, th, col, border=1, align='C')
        pdf.ln(th + padding)

        pdf.set_font("Arial", '', 10)
        for i, row in df_crosstab.iterrows():
            for value in row:
                pdf.cell(col_width, th, str(value), border=1, align='C')
            pdf.ln(th + padding)
        pdf.ln(10)
    ...
    if freq:
        pdf.add_page()
        pdf.set_font("Arial", 'B', 16)
        pdf.cell(200, 10, txt="Frequency Table", ln=True, align='L')
        pdf.ln(10)

        df_freq = pd.read_csv(freq)
        col_width = 170 / len(df_freq.columns)
        th = pdf.font_size + 2
        padding = 1.5
    ...
    if attack_rate:
        pdf.add_page()
        pdf.set_font("Arial", 'B', 16)
        pdf.cell(200, 10, txt="Attack Rate Table", ln=True, align='L')
        pdf.ln(10)

        df_attack_rate = pd.read_csv(attack_rate)
        col_width = 170 / len(df_attack_rate.columns)
        th = pdf.font_size + 2
        padding = 1.5

        pdf.set_font("Arial", 'B', 10)
        for col in df_attack_rate.columns:
            pdf.cell(col_width, th, col, border=1, align='C')
        pdf.ln(th + padding)

        pdf.set_font("Arial", '', 10)
        for i, row in df_attack_rate.iterrows():
            for value in row:
                pdf.cell(col_width, th, str(value), border=1, align='C')
            pdf.ln(th + padding)
    
    pdf.output(report)
    click.echo(f"Report saved as {report}")
